...
import { isCorrectSpelling, WordEntry, CHARG_WORD, liveFeedbackCharMatches, liveFeedbackContainsChar, resolveWordForActiveModifiers } from "@/data/words";
...
const CustomGame = ({ customWords, userId, activeSound, activeFont, keyboardLayout, keySize = 15, restartKeybind }: Props) => {
  const [currentWord, setCurrentWord] = useState<WordEntry>(() => 
    customWords.length > 0 ? customWords[Math.floor(Math.random() * customWords.length)] : { primary: "word", alternates: [] }
  );
  ...
  const nextWord = useCallback(() => {
    ...
    const next = customWords[Math.floor(Math.random() * customWords.length)];
    setCurrentWord(next);
    setInput("");
    setResult("idle");
    setStartTime(null);
