...
import { getRandomGeniusWord, isCorrectSpelling, WordEntry, liveFeedbackCharMatches, liveFeedbackContainsChar, resolveWordForActiveModifiers } from "@/data/words";
...
const GeniusGame = ({ userId, activeSound, activeFont, keyboardLayout, keySize = 15, restartKeybind }: Props) => {
  const [currentWord, setCurrentWord] = useState<WordEntry>(() => getRandomGeniusWord());
  ...
  const nextWord = useCallback(() => {
    const next = getRandomGeniusWord(currentWord.primary);
    setCurrentWord(next);
    setInput("");
    setResult("idle");
    setStartTime(null);
