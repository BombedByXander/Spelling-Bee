...
import { getRandomModerateWord, isCorrectSpelling, WordEntry, liveFeedbackCharMatches, liveFeedbackContainsChar, resolveWordForActiveModifiers } from "@/data/words";
...
const ModerateGame = ({ userId, activeSound, activeFont, keyboardLayout, keySize = 15, restartKeybind }: Props) => {
  const [currentWord, setCurrentWord] = useState<WordEntry>(() => getRandomModerateWord());
  ...
  const nextWord = useCallback(() => {
    const next = getRandomModerateWord(currentWord.primary);
    setCurrentWord(next);
    setInput("");
    setResult("idle");
    setStartTime(null);
  ...
  useEffect(() => {
    ...
    if (correct) {
      ...
      let earned = 10;
      if (newStreak > 0 && newStreak % 10 === 0) earned += 30;
      if (userId) {
  ...
  const handleClick = (e: MouseEvent) => {
    ...
    if (!target) return;
    // don't steal clicks for interactive elements (dropdowns, buttons, inputs, selects)
