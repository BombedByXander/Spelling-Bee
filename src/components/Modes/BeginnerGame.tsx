...
import { getRandomBeginnerWord, isCorrectSpelling, WordEntry, liveFeedbackCharMatches, liveFeedbackContainsChar, resolveWordForActiveModifiers } from "@/data/words";
...
const BeginnerGame = ({ userId, activeSound, activeFont, keyboardLayout, keySize = 15, restartKeybind }: Props) => {
  const [currentWord, setCurrentWord] = useState<WordEntry>(() => getRandomBeginnerWord());
  ...
  const nextWord = useCallback(() => {
    const next = getRandomBeginnerWord(currentWord.primary);
    setCurrentWord(next);
    setInput("");
    setResult("idle");
    setStartTime(null);
