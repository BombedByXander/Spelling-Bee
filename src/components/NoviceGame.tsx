...
import { getRandomNoviceWord, isCorrectSpelling, WordEntry, liveFeedbackCharMatches, liveFeedbackContainsChar, resolveWordForActiveModifiers } from "@/data/words";
...
const NoviceGame = ({ userId, activeSound, activeFont, keyboardLayout, keySize = 15, restartKeybind }: Props) => {
  const [currentWord, setCurrentWord] = useState<WordEntry>(() => getRandomNoviceWord());
  ...
  const nextWord = useCallback(() => {
    const next = getRandomNoviceWord(currentWord.primary);
    setCurrentWord(next);
    setInput("");
    setResult("idle");
    setStartTime(null);
